...
BSTree SearchBST(BSTree T,ElemType key){//二叉查找树的查找
    if(!T||key==T->data)//若查找成功或空，返回T 
        return T;
    if(key<T->data)
        SearchBST(T->lchild,key);//在左子树中继续查找
    else
        SearchBST(T->rchild,key); //在右子树中继续查找
}
...
void CreateBST(BSTree &T){//二叉查找树的创建
    T=NULL;
    ElemType e;
    cin>>e; //依次读入关键字，插入二叉查找树中
    while(e!=ENDFLAG){//ENDFLAG为自定义常量，作为输入结束标志
        InsertBST(T,e);  //插入二叉查找树中
        cin>>e;
	}
}

void DeleteBST(BSTree &T,ElemType key){//二叉查找树的删除
    ...
    if(p->lchild&&p->rchild){//被删结点p左右子树均不空
        ...
        if(q!=p)
            ...
            q->lchild=s->lchild; //重接q的左子树（特殊情况） 
        delete s;
    }
    else{
        q=p;
...
int main(){
	BSTree T;
	ElemType key;
	cout<<"创建二叉查找树，请输入一些整型数，-1结束"<<endl;
	CreateBST(T);
	cout<<"当前二叉查找树中序遍历结果为"<<endl;
	InOrderTraverse(T);
	cout<<endl;
	cout<<"请输入待查找关键字"<<endl;
	cin>>key;
